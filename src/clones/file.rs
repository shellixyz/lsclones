...
use std::{path::{PathBuf, Path}, collections::HashMap, io};

use anyhow::anyhow;
use path_absolutize::Absolutize;

use crate::path::{HashedAbsolutePathSet, HashedAbsolutePath};
...
impl File {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> anyhow::Result<Self> {
        ...
        let content: HashMap<String, serde_json::Value> = serde_json::from_reader(file_buf)
            .map_err(|err| anyhow!("failed opening clones file {}: {err}", path.as_ref().to_string_lossy()))?;
        Ok(Self { path: path.as_ref().to_path_buf(), content })
    }

    fn missing_item(&self, section: &str) -> anyhow::Error {
        anyhow!("could not find {section} section in dups file: {}", self.path.to_string_lossy())
    }
        ...
        for path_json_value in json_paths {
            match path_json_value {
                serde_json::Value::String(path_str) => {
                    let dir_path = Path::new(path_str).absolutize_from(base_dir).unwrap().to_path_buf();
        ...
        for json_clone_group in json_clone_groups {
            match json_clone_group {
                serde_json::Value::Object(json_clone_group) => {
                    ...
                    let file_len = if let Some(serde_json::Value::Number(file_len)) = json_clone_group.get("file_len") {
                        file_len.as_u64().ok_or_else(|| anyhow!("bad group/file_len value: {file_len:?}"))?
                    } else {
                        return Err(self.missing_item("group/file_len"))
                    };

                    let Some(serde_json::Value::Array(files)) = json_clone_group.get("files") else {
                    ...
                    if group_files.len() < 2 {
                        log::warn!("found group with less than 2 files in {}", self.path.to_string_lossy());
                    }
