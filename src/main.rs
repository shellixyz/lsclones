...
use std::{io::{Write, self}, path::{Path}, os::unix::prelude::MetadataExt, cmp::Ord, env::current_dir, process, borrow::Borrow};

use clap::Parser;
use cli::{CommonOptions, CommandArgsPaths};
use clones::db::ClonesDB;
use crossterm::cursor;
use env_logger::fmt::Color;
use fs::{dir, tree::{clones::{CloneDir, CloneDirs, RefDir}}};
use itertools::Itertools;
use paths::{TreeWithProgress, Clones, PathRefs};
use size::Size;
...
use crate::{
    cli::{Cli, Commands},
    error_behavior::ErrorBehavior, fs::tree::clones::CloneDirGroups,
};


fn files_command(args: &Commands, clones_db: &ClonesDB) -> anyhow::Result<()> {
    let Commands::Files {
        global_options, unique, paths,
        map, inside, outside, inside_only, null_line_terminator,
    ...
    if *map {
        ...
        for (index, clone_group) in clone_groups.iter().enumerate() {
            if (! (*inside || *inside_only) || clone_group.inside().len() > 1) && !(*outside && clone_group.outside().is_empty()) {
                for file in clone_group.inside().iter() { print_path(file, path_print_style, *null_line_terminator); }
                ...
                if ! (*inside_only || clone_group.outside().is_empty()) {
                    bunt::eprintln!("{$green}=>{/$}");
                    for file in clone_group.outside().iter() { print_path(file, path_print_style, *null_line_terminator); }
                }
                if index < clone_groups.len() - 1 { eprintln!() }
            }
        }
        ...
        if display_stats {
            eprintln!();
            bunt::eprintln!("{[green]:} {$bold}files, total size{/$} {[green]:}", file_count, size::Size::from_bytes(total_size));
        }
...
fn print_clone_dir_path<'a>(clone_dir: impl Borrow<CloneDir<'a>>, style: PathPrintStyle) {
    ...
    if let PathPrintStyle::RelativeTo(prefix) = style {
        if let Ok(rel_path) = path.strip_prefix(prefix) { path = rel_path; }
    }
    if let Some(deep_path_rel) = clone_dir.deep_path_rel() {
        bunt::println!("{}{$cyan}/{}{/$}", path.to_string_lossy(), deep_path_rel.to_string_lossy());
    } else {
        println!("{}", path.to_string_lossy());
    }
}

fn print_ref_dir_path<'a>(ref_dir: impl Borrow<RefDir<'a>>, style: PathPrintStyle, missing_files: usize, extra_files: usize) {
    let mut path = ref_dir.borrow().path().as_path();
    if let PathPrintStyle::RelativeTo(prefix) = style {
        path = path.strip_prefix(prefix).unwrap_or(path);
    }
    bunt::println!("{} {$bold}({[green]:} missing, {[green]:} extra){/$}", path.to_string_lossy(), missing_files, extra_files);
}

#[derive(Debug, Clone, Copy)]
enum RefFileType { Extra, Missing }
...
fn dirs_command_clones(dirs: PathRefs, global_options: &CommonOptions, map: bool, show_refs: bool, ref_details: bool, error_behavior: ErrorBehavior, clones_db: &ClonesDB, null_line_terminator: bool) -> anyhow::Result<()> {
    ...
    if map {
        ...
        let clone_dir_groups =
            dirs
            .iter()
            .flat_map(|dir| file_tree.clone_dir_groups(dir, clones_db, global_options.recursive()).unwrap())
            .collect::<CloneDirGroups>();


        for (index, clone_dir_group) in clone_dir_groups.iter().enumerate() {
            for dir in clone_dir_group { print_clone_dir_path(dir, path_print_style); }
            if show_refs || ref_details {
                ...
                if ! ref_dirs.is_empty() {
                    ...
                    for ref_dir in ref_dirs {
                        ...
                        print_ref_dir_path(&ref_dir, path_print_style, missing_files.len(), extra_files.len());
                        if ref_details {
                            for file in missing_files { print_ref_file(file, path_print_style, RefFileType::Missing) }
                            for file in extra_files { print_ref_file(file, path_print_style, RefFileType::Extra) }
                        }
                    }
                }
            }
            if index < clone_dir_groups.len() - 1 { println!() }
        }
        ...
        let mut clone_dirs =
            dirs
            .iter()
            .flat_map(|dir| file_tree.clone_dirs(dir, clones_db, global_options.recursive()).unwrap())
            .collect::<CloneDirs>();
        ...
        if global_options.stats() {
            eprintln!();
            bunt::eprintln!("{[green]:} {$bold}dirs, total size{/$} {[green]:}", clone_dirs.len(), clone_dirs.size_human());
        }
    }
...
fn dirs_command_unique(dirs: PathRefs, global_options: &CommonOptions, clones_db: &ClonesDB, null_line_terminator: bool) -> anyhow::Result<()> {
    let mut udirs = vec![];
    let mut dir_count = 0;
    let mut total_size = 0;
    ...
    if global_options.stats() {
        eprintln!();
        bunt::eprintln!("{[green]:} {$bold}dirs, total size{/$} {[green]:}", dir_count, size::Size::from_bytes(total_size));
    }
...
fn dirs_command(args: &Commands, clones_db: &ClonesDB) -> anyhow::Result<()> {
    let Commands::Dirs {
        dirs, map, show_refs, ref_details,
        unique, global_options, error_behavior,
    ...
    if *unique {
        ...
        dirs_command_clones(dirs, global_options, *map, *show_refs, *ref_details, *error_behavior, clones_db, *null_line_terminator)?;
    }
